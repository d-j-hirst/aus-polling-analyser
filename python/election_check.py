...
def check_seat_fp_percent_total(elections):
    for code, election in elections.elections.items():
...
def check_vote_fp_percent_calc(elections):
    for code, election in elections.elections.items():
        for seat_result in election.seat_results:
            if len(seat_result.fp) < 2:
                continue  # Don't need to flag some old uncontested seats
...
def check_seat_tcp_percent_total(elections):
    for code, election in elections.elections.items():
        for seat_result in election.seat_results:
...
if __name__ == '__main__':
    ...
    check_seat_fp_percent_total(elections)
    check_vote_fp_percent_calc(elections)
    check_seat_tcp_percent_total(elections)
