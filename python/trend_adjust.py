...
def print_smoothed_series(config, label, some_dict, file, force_monotone=False, bounds=[-math.inf, math.inf]):
...
def getBiasData(inputs, poll_trend, party_group, day, avg_n, studied_election):
    biasData = BiasData()
...
def get_single_election_data(inputs, poll_trend, party_group, day_data, day, avg_n, studied_election, mix_limits):
    biasData = getBiasData(inputs=inputs,
                            poll_trend=poll_trend,
                            party_group=party_group,
                            day=day,
                            avg_n=avg_n,
                            studied_election=studied_election)
    previous_bias = statistics.median(biasData.previous_errors)
    poll_bias = statistics.median(biasData.poll_errors)
    ...
    if studied_election == no_target_election_marker:
        return
    if biasData.studied_previous_error is not None:
        previous_debiased_error = biasData.studied_previous_error - previous_bias
        if mix_limits == (0, 1):
            day_data.previous_debiased_errors.append(previous_debiased_error)
    if len(biasData.studied_poll_errors) > 0:
        for studied_poll_error, studied_poll_party in zip(biasData.studied_poll_errors, biasData.studied_poll_parties):
...
def get_day_data(inputs, poll_trend, party_group, day, avg_n):
    ...
    while mix_limits[1] - mix_limits[0] > 0.0001:
        ...
        for mix_index in range(0, len(mix_limits)):
            mixed_deviation = smoothed_median([abs(a) for a in day_data.mixed_errors[mix_index]], 2)
            mixed_rmse = math.sqrt(sum([a ** 2 for a in day_data.mixed_errors[mix_index]]) / len(day_data.mixed_errors[mix_index]))
...
def get_party_data(config, inputs, poll_trend, party_group, avg_n):
    ...
    for day in config.days:
        day_data = get_day_data(inputs=inputs,
                                ...
                                avg_n=avg_n)
        if day == 0:
            previous_bias = smoothed_median(day_data.previous_debiased_errors, 2)
        poll_bias = smoothed_median(day_data.overall_poll_biases, 2)
        previous_bias = smoothed_median(day_data.overall_previous_biases, 2)
        mixed_bias = smoothed_median(day_data.mixed_errors[1], 2)
        mixed_deviation = smoothed_median([abs(a) for a in day_data.mixed_errors[1]], 2)
        lower_errors = [a - mixed_bias for a in day_data.mixed_errors[1] if a < mixed_bias]
        upper_errors = [a - mixed_bias for a in day_data.mixed_errors[1] if a > mixed_bias]
...
def save_party_data(config, party_data, exclude, party_group):
    ...
    with open(filename, 'w') as f:
        print_smoothed_series(config, 'Poll Bias', party_data.poll_biases, f)
        print_smoothed_series(config, 'Previous-Elections Bias', party_data.previous_biases, f)
        print_smoothed_series(config, 'Mixed Bias', party_data.biases, f)
        print_smoothed_series(config, 'Lower Error', party_data.lower_rmses, f, force_monotone=True, bounds=(0, math.inf))
        print_smoothed_series(config, 'Upper Error', party_data.upper_rmses, f, force_monotone=True, bounds=(0, math.inf))
        print_smoothed_series(config, 'Lower Kurtosis', party_data.lower_kurtoses, f, bounds=(3, math.inf))
        print_smoothed_series(config, 'Upper Kurtosis', party_data.upper_kurtoses, f, bounds=(3, math.inf))
        print_smoothed_series(config, 'Mix factor', party_data.final_mix_factors, f, force_monotone=True, bounds=(0, 1))
