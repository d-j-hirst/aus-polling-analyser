...
inline float stdDevSingleSeat(float percentCounted) {
	...
	// but we multiply by the extra 1.5f here to account for pre-polling changes
	// and differences between state and federal elections
	return 4.2106f * 1.5f * pow(percentCounted, -0.522f);
}
...
inline float stdDevOverall(float percentCounted) {
	...
	// but we multiply by the extra 1.5f here to account for pre-polling changes
	// and differences between state and federal elections
	return 0.5403f * 1.5f * pow(percentCounted, -0.391f);
}