	...
	isNewPoll(isNewPoll), parent(parent), project(project), poll(poll)
{
	...
	int partyCount = project->getPartyCount() + 1; // includes "others".
	...
	for (; i < partyCount; i++) {
		if (i < partyCount - 1) {
			...
			partyString[i] = project->getParty(i).name + " primary vote:";
		}
		else {
...
void EditPollFrame::updateTextPrimary(wxCommandEvent& event) {
	...
	int partyPollIndex = (whichParty == project->getPartyCount() ? 15 : whichParty);
