...
void LivePreparation::prepareLiveFpSwings()
{
	for (auto const& [seatId, seat] : currentElection.seats) {
		...
		for (auto [candidate, swing] : seat.fpSwing) {
			...
			if (seat.name == "Tamworth") {
				logger << "TAMWORTH TROUBLE\n";
				PA_LOG_VAR(seat.name);
				PA_LOG_VAR(partyIndex);
				PA_LOG_VAR(currentElection.candidates[candidate].party);
				PA_LOG_VAR(aecPartyToSimParty);
				PA_LOG_VAR(run.liveSeatFpSwing[seatIndex][OthersIndex]);
				PA_LOG_VAR(run.liveSeatFpTransformedSwing[seatIndex][OthersIndex]);
				PA_LOG_VAR(run.liveSeatFpPercent[seatIndex][OthersIndex]);
			}
			if (seat.name == "Sydney") {
				logger << "SYDNEY STRESS\n";
				PA_LOG_VAR(run.liveSeatMajorFpDiff[seatIndex]);
			}
		}
		if (indFps.size()) {
			...
			if (seat.name == "Tamworth") {
				logger << "TAMWORTH TROUBLE 2\n";
				PA_LOG_VAR(voteShare);
				PA_LOG_VAR(swing);
				PA_LOG_VAR(transformedSwing);
				PA_LOG_VAR(run.liveSeatFpSwing[seatIndex][OthersIndex]);
				PA_LOG_VAR(run.liveSeatFpTransformedSwing[seatIndex][OthersIndex]);
				PA_LOG_VAR(run.liveSeatFpPercent[seatIndex][OthersIndex]);
			}
