...
enum {
	...
	PA_MapFrame_SelectSimulationID
};
	...
	parent(parent)
{
	...
	Bind(wxEVT_COMBOBOX, &MapFrame::OnSimulationSelection, this, PA_MapFrame_SelectSimulationID);
...
void MapFrame::render(wxDC& dc) {
	...
	Simulation* sim = project->getSimulationPtr(selectedSeat);
...
void MapFrame::refreshToolbar() {
	...
	if (selectedSeat >= int(simulationArray.size())) {
		selectedSeat = int(simulationArray.size()) - 1;
	}
	if (selectedSeat >= 0) {
		comboBoxString = simulationArray[selectedSeat];
	}

	selectSeatComboBox = new wxComboBox(toolBar, PA_MapFrame_SelectSimulationID, comboBoxString, wxPoint(0, 0), wxSize(150, 30), simulationArray);
