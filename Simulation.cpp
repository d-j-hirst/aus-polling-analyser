...
void Simulation::run(PollingProject& project) {
	...
	for (auto thisSeat = project.getSeatBegin(); thisSeat != project.getSeatEnd(); ++thisSeat) {
		...
		if (live) determineSeatCachedBoothData(project, *thisSeat);
	}

	if (live) determinePpvcBias();
...
Simulation::SeatResult Simulation::calculateLiveResultClassic2CP(PollingProject const& project, Seat const& seat, float priorMargin)
{
	if (live && seat.latestResults && seat.latestResults->total2cpVotes()) {
		...
		if (seat.previousResults) {
			...
			int recordedFirstAbsentVotes = seat.latestResults->finalCandidates[0].absentVotes;
			int recordedSecondAbsentVotes = seat.latestResults->finalCandidates[1].absentVotes;
			int estimatedRemainingAbsentVotes = std::max(0, estimatedAbsentVotes - recordedFirstAbsentVotes - recordedSecondAbsentVotes);
			int remainingFirstAbsentVotes = int(std::round(firstNewAbsentPercent * float(estimatedRemainingAbsentVotes) * 0.01f));
			int remainingSecondAbsentVotes = estimatedRemainingAbsentVotes - remainingFirstAbsentVotes;
			int firstAbsentVotes = remainingFirstAbsentVotes + recordedFirstAbsentVotes;
			int secondAbsentVotes = recordedSecondAbsentVotes + remainingSecondAbsentVotes;
			...
			int recordedFirstProvisionalVotes = seat.latestResults->finalCandidates[0].provisionalVotes;
			int recordedSecondProvisionalVotes = seat.latestResults->finalCandidates[1].provisionalVotes;
			int estimatedRemainingProvisionalVotes = std::max(0, estimatedProvisionalVotes - recordedFirstProvisionalVotes - recordedSecondProvisionalVotes);
			int remainingFirstProvisionalVotes = int(std::round(firstNewProvisionalPercent * float(estimatedRemainingProvisionalVotes) * 0.01f));
			int remainingSecondProvisionalVotes = estimatedRemainingProvisionalVotes - remainingFirstProvisionalVotes;
			int firstProvisionalVotes = remainingFirstProvisionalVotes + recordedFirstProvisionalVotes;
			int secondProvisionalVotes = recordedSecondProvisionalVotes + remainingSecondProvisionalVotes;
			...
			int recordedFirstPrepollVotes = seat.latestResults->finalCandidates[0].prepollVotes;
			int recordedSecondPrepollVotes = seat.latestResults->finalCandidates[1].prepollVotes;
			int estimatedRemainingPrepollVotes = std::max(0, estimatedPrepollVotes - recordedFirstPrepollVotes - recordedSecondPrepollVotes);
			int remainingFirstPrepollVotes = int(std::round(firstNewPrepollPercent * float(estimatedRemainingPrepollVotes) * 0.01f));
			int remainingSecondPrepollVotes = estimatedRemainingPrepollVotes - remainingFirstPrepollVotes;
			int firstPrepollVotes = remainingFirstPrepollVotes + recordedFirstPrepollVotes;
			int secondPrepollVotes = recordedSecondPrepollVotes + remainingSecondPrepollVotes;
			...
			int recordedFirstPostalVotes = seat.latestResults->finalCandidates[0].postalVotes;
			int recordedSecondPostalVotes = seat.latestResults->finalCandidates[1].postalVotes;
			int estimatedRemainingPostalVotes = std::max(0, estimatedPostalVotes - recordedFirstPostalVotes - recordedSecondPostalVotes);
			int remainingFirstPostalVotes = int(std::round(firstNewPostalPercent * float(estimatedRemainingPostalVotes) * 0.01f));
			int remainingSecondPostalVotes = estimatedRemainingPostalVotes - remainingFirstPostalVotes;
			int firstPostalVotes = remainingFirstPostalVotes + recordedFirstPostalVotes;
			int secondPostalVotes = recordedSecondPostalVotes + remainingSecondPostalVotes;
