...
model {
    ...
    nswSwingDev[1:dayCount] * 0.3173 + vicSwingDev[1:dayCount] * 0.2556 +
        ...
        saSwingDev[1:dayCount] * 0.0749 + tanSwingDev * 0.05 ~ normal(0.0, 0.001);
    ...
    for (poll in 1:pollCount) {
      ...
      qldSwingDevPoll[poll] ~ normal(qldSwingDev[pollDay[poll]], distSigma * 1.254);
      if (waSwingDevPoll[poll] > -100) waSwingDevPoll[poll] ~ normal(waSwingDev[pollDay[poll]], distSigma * 1.777);
      if (saSwingDevPoll[poll] > -100) saSwingDevPoll[poll] ~ normal(saSwingDev[pollDay[poll]], distSigma * 2.058);
      if (wstanSwingDevPoll[poll] > -100) {
        wstanSwingDevPoll[poll] ~ normal(
          waSwingDev[pollDay[poll]] * 0.4459 + saSwingDev[pollDay[poll]] * 0.3323 + tanSwingDev[pollDay[poll]] * 0.2218,
          distSigma * 2.519
        );
      }
    }
