...
constexpr int MaxFpReconciliationCycles = 25;
...
namespace {
	struct InvalidIteration {
		...
		void addContext(std::string newContext)
		{
			if (!context.empty()) newContext += ", " + context;
		...
		std::string description() const
		{
			...
			std::string result =
				fileName + ":" + std::to_string(source.line()) +
				" in " + source.function_name();
	...
	float effectiveLiveConfidence(float completion, float confidence)
	{
		if (!std::isfinite(completion) || !std::isfinite(confidence) ||
			completion < 0.0f || completion > 1.0f ||
			confidence < 0.0f || confidence > 1.0f) {
			throw InvalidIteration();
		}
		return std::max(confidence, completion * completion);
	}

	float liveBlendWeight(
		float completion,
		float confidence,
		float slope = 12.0f)
	{
		float const effectiveConfidence =
			effectiveLiveConfidence(completion, confidence);
		return LiveSimulationMath::evidenceWeight(
			effectiveConfidence, slope);
	}

	bool materiallyNonZero(float value)
	{
		return std::abs(value) > LiveSimulationMath::EvidenceEpsilon;
	}
}
...
void SimulationIteration::reset()
{
	...
	indAlpha = 1.0f;
	indBeta = 1.0f;
	neededTerminalFpReconciliation = false;
...
int SimulationIteration::runIteration()
{
	...
	auto retryOrThrow = [&]() {
		if (retryCount >= MaxIterationRetries) {
			...
			if (!invalidReasons.empty()) {
				message += "\nInvalid states encountered:";
				for (auto const& [reason, count] : invalidReasons) {
					message +=
						"\n- " + reason + " (" +
						std::to_string(count) + " attempt" +
						(count == 1 ? ")" : "s)");
				}
			}
	...
	if (neededTerminalFpReconciliation) {
		run.recordWarning(
			...
			iterationIndex,
			"Regular FP reconciliation left an error above 0.6 before "
			"the bounded terminal solver.");
	}
	std::lock_guard<std::mutex> lock(recordMutex);
	...
	float preferenceFlowDeviation)
{
	...
	auto usesTacticalPreferenceFlow = [&](int partyIndex) {
		// An emerging party may occupy any ideological position, so it should
		// retain its sampled party-wide flow rather than inherit behaviour
		// calibrated for independents and broadly centrist candidates.
		return partyIndex != EmergingPartyIndex &&
			(partyIndex == OthersIndex ||
				partyIndex == EmergingIndIndex ||
				partyIdeologies[partyIndex] == 2);
	};
	...
	for (auto [partyIndex, voteShare] : run.pastSeatResults[seatIndex].fpVotePercent) {
		...
		if (voteShare > 5.0f && usesTacticalPreferenceFlow(partyIndex)) {
	...
	// Estimate FPs by removing expected preferences from expected TPP. Direct
	// subtraction is exact when both resulting major-party votes are valid. The
	// transformed path remains a bounded fallback for temporarily incompatible
	// seat FP/TPP combinations.
	...
	float const directPartyOneFp =
		partyOneScaledTpp - overallAdjustedPartyOnePrefs;
	float const directPartyTwoFp =
		partyTwoScaledTpp - overallAdjustedPartyTwoPrefs;
	constexpr float MinMajorPartyFp = 0.1f;
	bool const directAllocationIsValid =
		directPartyOneFp >= MinMajorPartyFp &&
		directPartyTwoFp >= MinMajorPartyFp;
	...
	float fallbackPartyOneFp = newPartyOneFp;
	float fallbackPartyTwoFp = newPartyTwoFp;

	float addedPartyOneFp = fallbackPartyOneFp;
	float addedPartyTwoFp = fallbackPartyTwoFp;
	if (addPartyOneFp >= 0.0f) {
		addedPartyOneFp = basicTransformedSwing(fallbackPartyOneFp, addPartyOneFp);
	}
	else {
		addedPartyTwoFp = basicTransformedSwing(fallbackPartyTwoFp, addPartyTwoFp);
	}

	fallbackPartyOneFp = std::clamp(
		addedPartyOneFp, MinMajorPartyFp, 99.9f);
	fallbackPartyTwoFp = std::clamp(
		addedPartyTwoFp, MinMajorPartyFp, 99.9f);
	float finalPartyOneFp = directAllocationIsValid ?
		directPartyOneFp : fallbackPartyOneFp;
	float finalPartyTwoFp = directAllocationIsValid ?
		directPartyTwoFp : fallbackPartyTwoFp;
...
void SimulationIteration::allocateMajorPartyFpsWithCompatibleTpp()
{
	...
	// only those seat TPPs back into the feasible range, redistributing the
	// change across other seats so the turnout-weighted overall TPP is retained.
	constexpr float MinMajorPartyFp = 0.1f;
	...
	auto const overallTppMatchesTarget = [&]() {
		...
		for (int seatIndex = 0; seatIndex < seatCount; ++seatIndex) {
			double const seatTpp =
				double(partyOneNewTppMargin[seatIndex]) + 50.0;
			double const turnout = std::max(
				double(run.pastSeatResults[seatIndex].turnoutCount), 0.0);
			weightedTpp += seatTpp * turnout;
			totalWeight += turnout;
			unweightedTpp += seatTpp;
		}
		double const averageTpp = totalWeight > 0.0 ?
	...
	for (int pass = 0; pass < MaxCompatibilityPasses; ++pass) {
		...
		bool const overallTppPreserved =
			pass == 0 || overallTppMatchesTarget();
		...
		if (pass + 1 == MaxCompatibilityPasses) {
			// An infeasible aggregate set of bounds must not silently change the
			// simulation's overall TPP. Restore the original TPPs and retain the
			// established transformed fallback for affected seats.
		...
		std::vector<double> lowerTpp(seatCount);
		std::vector<double> adjustedTpp(seatCount);
		std::vector<double> upperTpp(seatCount);
		...
		for (int seatIndex = 0; seatIndex < seatCount; ++seatIndex) {
			...
			double const nonExhaustedVote =
				double(allocation.nonExhaustedVote);
			if (!std::isfinite(nonExhaustedVote) ||
				nonExhaustedVote <= 0.0) {
				throw InvalidIteration();
			}

			lowerTpp[seatIndex] = std::clamp(
				currentTpp +
					double(MinMajorPartyFp -
						allocation.directPartyOneFp) *
					100.0 / nonExhaustedVote,
				0.1, 99.9);
			upperTpp[seatIndex] = std::clamp(
				currentTpp +
					double(allocation.directPartyTwoFp -
						MinMajorPartyFp) *
					100.0 / nonExhaustedVote,
				0.1, 99.9);
			if (lowerTpp[seatIndex] > upperTpp[seatIndex]) {
				// Leave an impossible seat on the transformed fallback path.
				lowerTpp[seatIndex] = currentTpp;
				upperTpp[seatIndex] = currentTpp;
			}
			adjustedTpp[seatIndex] = std::clamp(
				currentTpp, lowerTpp[seatIndex], upperTpp[seatIndex]);
			turnoutWeight[seatIndex] = std::max(
				double(run.pastSeatResults[seatIndex].turnoutCount), 0.0);
			totalWeight += turnoutWeight[seatIndex];
		}
		if (totalWeight <= 0.0) {
			std::fill(turnoutWeight.begin(), turnoutWeight.end(), 1.0);
			totalWeight = double(seatCount);
		}

		double const targetWeightedTpp =
			double(iterationOverallTpp) * totalWeight;
		for (int redistribution = 0;
			redistribution <= seatCount; ++redistribution) {
			double currentWeightedTpp = 0.0;
			for (int seatIndex = 0; seatIndex < seatCount; ++seatIndex) {
				currentWeightedTpp +=
					adjustedTpp[seatIndex] * turnoutWeight[seatIndex];
			}
			double const remaining =
				targetWeightedTpp - currentWeightedTpp;
			if (std::abs(remaining) < 0.0001 * totalWeight) break;

			bool const increase = remaining > 0.0;
			double adjustableWeight = 0.0;
			for (int seatIndex = 0; seatIndex < seatCount; ++seatIndex) {
				double const headroom = increase ?
					upperTpp[seatIndex] - adjustedTpp[seatIndex] :
					adjustedTpp[seatIndex] - lowerTpp[seatIndex];
				if (headroom > 0.000001) {
					adjustableWeight += turnoutWeight[seatIndex];
				}
			}
			if (adjustableWeight <= 0.0) break;

			double const commonAdjustment =
				remaining / adjustableWeight;
			for (int seatIndex = 0; seatIndex < seatCount; ++seatIndex) {
				double const headroom = increase ?
					upperTpp[seatIndex] - adjustedTpp[seatIndex] :
					adjustedTpp[seatIndex] - lowerTpp[seatIndex];
				if (headroom <= 0.000001) continue;
				double const adjustment = increase ?
					std::min(commonAdjustment, headroom) :
					std::max(commonAdjustment, -headroom);
				adjustedTpp[seatIndex] += adjustment;
			}
		}

		for (int seatIndex = 0; seatIndex < seatCount; ++seatIndex) {
			partyOneNewTppMargin[seatIndex] =
				float(adjustedTpp[seatIndex] - 50.0);
		}
	}
}
...
void SimulationIteration::reconcileSeatAndOverallFp()
{
	for (int i = 0; i < MaxFpReconciliationCycles; ++i) {
		...
		if (i > 2) {
			correctMajorPartyFpBias();
			// The correction above mutates every seat. Refresh the aggregate so
	...
	// the seat TPP. Apply one terminal aggregate correction so that this final
	// reallocation does not leave a systematic major-party FP bias.
	calculateNewFpVoteTotals();
	if (hasInvalidValues("terminal FP reconciliation totals")) {
		throw InvalidIteration();
	}
	FloatByPartyIndex const preTerminalFp = tempOverallFp;
	float const preTerminalFpError = overallFpError;
	correctMajorPartyFpBias(true);
	if (hasInvalidValues("terminal major-party FP reconciliation")) {
		throw InvalidIteration();
	}
	calculateNewFpVoteTotals();
	...
	// tempOverallFp and overallFpError stale.
	...
	calculateNewFpVoteTotals(
		true, &preTerminalFp, preTerminalFpError);
}

void SimulationIteration::solveTerminalFpReconciliation()
{
	if (overallFpError <= AcceptableOverallFpError) return;
	neededTerminalFpReconciliation =
	...
	FloatByPartyIndex cumulativeMultipliers;
	for (auto const& [partyIndex, target] : reconciliationTargets) {
		if (!isMajor(partyIndex)) {
			cumulativeMultipliers[partyIndex] = 1.0f;
		}
	}
	if (cumulativeMultipliers.empty()) return;

	struct ReconciliationShares {
		FloatByPartyIndex adjustable;
		FloatByPartyIndex protectedVote;
	};

	auto const categoryForParty = [&](int partyIndex) {
		if (run.natPartyIndex >= 0 &&
	...
	auto const isAdjustable = [&](int partyIndex, int category,
		int incumbentPartyIndex) {
		if (isMajor(category)) return false;
		if (category != OthersIndex) {
			return partyIndex != incumbentPartyIndex;
		}
		if (partyIndex == run.indPartyIndex) return false;
		if (partyIndex >= Mp::Others &&
			!overallFpSwing.contains(partyIndex)) {
			return false;
		}
		if (partyIndex == incumbentPartyIndex) return false;
		return partyIndex == OthersIndex ||
			!overallFpTarget.contains(partyIndex);
	};
	auto const measureShares = [&]() {
		ReconciliationShares result;
		...
		for (int seatIndex = 0;
			...
			for (auto const& [partyIndex, voteShare] :
				...
				int const category = categoryForParty(partyIndex);
				if (!cumulativeMultipliers.contains(category)) continue;
				auto& destination = isAdjustable(
					partyIndex, category, incumbentPartyIndex) ?
					result.adjustable : result.protectedVote;
				destination[category] += float(voteCount);
			}
		}
		if (!std::isfinite(totalVoteCount) || totalVoteCount <= 0.0) {
			throw InvalidIteration();
		}
		for (auto& [partyIndex, voteCount] : result.adjustable) {
			voteCount =
				float(double(voteCount) / totalVoteCount * 100.0);
		}
		for (auto& [partyIndex, voteCount] : result.protectedVote) {
			voteCount =
				float(double(voteCount) / totalVoteCount * 100.0);
		}
		return result;
	...
	auto const baseSeatFp = seatFpVoteShare;
	auto const baseSeatTppMargins = partyOneNewTppMargin;
	auto bestSeatFp = seatFpVoteShare;
	auto bestSeatTppMargins = partyOneNewTppMargin;
	// The incoming aggregate correction has not yet been reconstructed against
	...
	for (int cycle = 0;
		...
		auto const shares = measureShares();
		bool adjustedAnyCategory = false;
		for (auto& [partyIndex, cumulativeMultiplier] :
			cumulativeMultipliers) {
			float const target =
				reconciliationTargets.at(partyIndex);
			float const protectedShare =
				getAt(shares.protectedVote, partyIndex, 0.0f);
			float const adjustableShare =
				getAt(shares.adjustable, partyIndex, 0.0f);
			if (adjustableShare <= 0.0001f) continue;

			float const desiredAdjustable =
				std::max(0.0f, target - protectedShare);
			float const rawAdjustment =
				desiredAdjustable / adjustableShare;
			if (!std::isfinite(rawAdjustment)) {
				throw InvalidIteration();
			}
			if (std::abs(rawAdjustment - 1.0f) < 0.0001f) continue;

			// Damping and cumulative bounds keep coupled party corrections
			// stable when a different locally strong party is fixed by seat
			// normalisation on successive trials.
			float const boundedAdjustment =
				std::clamp(rawAdjustment, 0.25f, 4.0f);
			float const dampedAdjustment =
				std::pow(boundedAdjustment, 0.75f);
			cumulativeMultiplier = std::clamp(
				cumulativeMultiplier * dampedAdjustment,
				0.05f, 20.0f);
		...
		for (int seatIndex = 0;
			...
			for (auto& [partyIndex, voteShare] :
				seatFpVoteShare[seatIndex]) {
				int const category = categoryForParty(partyIndex);
				if (!cumulativeMultipliers.contains(category) ||
					!isAdjustable(
						partyIndex, category, incumbentPartyIndex)) {
					continue;
				}
				voteShare *= cumulativeMultipliers.at(category);
				if (!std::isfinite(voteShare) || voteShare < 0.0f) {
					throw InvalidIteration();
				}
			}

			float nonMajorFp = 0.0f;
			for (auto const& [partyIndex, voteShare] :
				seatFpVoteShare[seatIndex]) {
				if (isMajor(partyIndex) ||
					(run.natPartyIndex >= 0 &&
				...
				nonMajorFp += voteShare;
			}
			constexpr float MaxNonMajorFp = 95.0f;
			if (nonMajorFp > MaxNonMajorFp) {
				float const factor = MaxNonMajorFp / nonMajorFp;
				for (auto& [partyIndex, voteShare] :
					seatFpVoteShare[seatIndex]) {
					if (isMajor(partyIndex) ||
						(run.natPartyIndex >= 0 &&
		...
		calculateNewFpVoteTotals();
		if (hasInvalidValues(
			"terminal bounded FP reconciliation, cycle " +
				std::to_string(cycle))) {
			throw InvalidIteration();
		}
		if (overallFpError + 0.0001f < bestError) {
			...
			bestSeatTppMargins = partyOneNewTppMargin;
			stagnantCycles = 0;
		}
		else {
			++stagnantCycles;
		}
		if (bestError < AcceptableOverallFpError ||
			stagnantCycles >= 3) {
			break;
		}
	}

	seatFpVoteShare = std::move(bestSeatFp);
	partyOneNewTppMargin = std::move(bestSeatTppMargins);
	calculateNewFpVoteTotals();
}

void SimulationIteration::calculateNewFpVoteTotals(
	bool finalCheckpoint,
	FloatByPartyIndex const* preTerminalFp,
	float preTerminalFpError)
{
	...
	float const literalOthers =
		float(literalOthersVoteCount / totalVoteCount * 100.0);
	...
	// A sub-3% major-party target can conflict with seat-level TPP constraints
	// and bounded preference flows. Keep the best reconciliation available, but
	// do not report these inherently exceptional draws as general failures.
	bool const hasExtremeMajorTarget =
		getAt(reconciliationTargets, Mp::One, 0.0f) < 3.0f ||
		getAt(reconciliationTargets, Mp::Two, 0.0f) < 3.0f;
	if (finalCheckpoint &&
		overallFpError > FpReconciliationWarningThreshold &&
		...
		logger << "\n*** FP reconciliation diagnostic: iteration "
			<< iterationIndex << ", final error above "
			<< FpReconciliationWarningThreshold
			<< ", retry " << retryCount << " ***\n";
		PA_LOG_VAR(reconciliationTargets);
		PA_LOG_VAR(tempOverallFp);
		if (preTerminalFp) {
			logger << *preTerminalFp << " - preTerminalFp; "
				<< preTerminalFpError << " - preTerminalFpError\n";
		}
		logger << iterationOverallTpp << " - iterationOverallTpp; "
		...
		logger << literalOthers << " - literalOthers; "
...
void SimulationIteration::applyCorrectionsToSeatFps()
{
	for (auto const& [partyIndex, vote] : tempOverallFp) {
		...
		if (partyIndex != OthersIndex) {
			...
			if (!std::isfinite(correctionFactor) ||
			...
			for (int seatIndex = 0; seatIndex < project.seats().count(); ++seatIndex) {
				...
				if (seatFpVoteShare[seatIndex].contains(partyIndex)) {
					...
					if (partyIndex == EmergingPartyIndex) {
						// Emerging-party support is often spread thinly across
						// many seats. The capped logit derivative would suppress
						// most of this aggregate correction at low seat shares.
						float const currentVote =
							seatFpVoteShare[seatIndex].at(partyIndex);
						float const correctedVote =
							correctionFactor < 1.0f ?
							currentVote * correctionFactor :
							currentVote + correctionSwing;
						if (!std::isfinite(correctedVote)) {
							throw InvalidIteration();
						}
						seatFpVoteShare[seatIndex][partyIndex] =
							std::clamp(correctedVote, 0.01f, 99.0f);
					}
					else {
						seatFpVoteShare[seatIndex][partyIndex] =
...
void SimulationIteration::correctMajorPartyFpBias(
	bool accountForSeatNormalisation)
{
	...
	// terminal checkpoint when seat-level normalization materially differs.
	...
	auto const partyAdjustments = [&](float factor) {
		float const partyOneTarget =
			partyOneCurrent * factor -
			partyOnePrefAdvantage * 0.005f;
		float const partyTwoTarget =
			partyTwoCurrent * factor +
			partyOnePrefAdvantage * 0.005f;
		return std::pair{
			partyOneTarget / partyOneCurrent,
			partyTwoTarget / partyTwoCurrent};
	};

	if (accountForSeatNormalisation) {
		// The closed-form factor above assumes election-wide normalisation.
		// Seats are actually normalised separately, sometimes while preserving
		// a strong local minor party, so solve the same one-dimensional problem
		// against the normalization that will really be applied.
		auto const projectedMajorFp = [&](float factor) {
			auto const [partyOneAdjust, partyTwoAdjust] =
				partyAdjustments(factor);
			...
			double weightedMajorFp = 0.0;
			double totalTurnout = 0.0;
			for (int seatIndex = 0;
				seatIndex < project.seats().count(); ++seatIndex) {
				auto const& seatFp = seatFpVoteShare[seatIndex];

				int fixedParty = -10000;
				float fixedVote = 0.0f;
				for (auto const& [partyIndex, voteShare] : seatFp) {
					if (partyIndex < Mp::Others) continue;
					if (partyIndex == run.indPartyIndex) continue;
					if (voteShare > fixedVote) {
						fixedVote = voteShare;
				...
				float const adjustedPartyOne =
					getAt(seatFp, Mp::One, 0.0f) * partyOneAdjust;
				float const adjustedPartyTwo =
					getAt(seatFp, Mp::Two, 0.0f) * partyTwoAdjust;
				float nonFixedTotal = 0.0f;
				for (auto const& [partyIndex, voteShare] : seatFp) {
					if (partyIndex == CoalitionPartnerIndex ||
						partyIndex == fixedParty ||
						partyIndex == Mp::One ||
						partyIndex == Mp::Two) {
						continue;
					}
					nonFixedTotal += voteShare;
				}
				float const totalToNormalise =
					nonFixedTotal + adjustedPartyOne + adjustedPartyTwo;
				float const normalisationTarget = 100.0f - fixedVote;
				if (!std::isfinite(totalToNormalise) ||
					totalToNormalise <= 0.0f ||
					!std::isfinite(normalisationTarget) ||
					normalisationTarget < 0.0f) {
					return -1.0f;
				}

				float const normalisationFactor =
					normalisationTarget / totalToNormalise;
				double const turnout =
					double(run.pastSeatResults[seatIndex].turnoutCount);
				weightedMajorFp +=
					double(adjustedPartyOne + adjustedPartyTwo) *
					double(normalisationFactor) * turnout;
				totalTurnout += turnout;
			}
			if (!std::isfinite(weightedMajorFp) ||
				!std::isfinite(totalTurnout) ||
				totalTurnout <= 0.0) {
				return -1.0f;
			}
			return float(weightedMajorFp / totalTurnout);
		};

		float minimumFactor = 0.0001f;
		float const preferenceAdjustment =
			partyOnePrefAdvantage * 0.005f;
		if (preferenceAdjustment > 0.0f) {
			minimumFactor = std::max(
				minimumFactor,
				preferenceAdjustment / partyOneCurrent + 0.0001f);
		}
		else if (preferenceAdjustment < 0.0f) {
			minimumFactor = std::max(
				minimumFactor,
	...
	auto const [partyOneAdjust, partyTwoAdjust] =
		partyAdjustments(adjustmentFactor);
	if (!std::isfinite(partyOneAdjust) ||
		!std::isfinite(partyTwoAdjust) ||
		partyOneAdjust <= 0.0f || partyTwoAdjust <= 0.0f) {
		throw InvalidIteration();
...
{
	if (!sim.isLive() || run.doingBettingOddsCalibrations ||
		run.doingLiveBaselineSimulation) {
		return;
	}
	if (!liveElection) {
		throw std::runtime_error(
			"Live result incorporation requires a generated live scenario.");
	}

	// Blend the prior seat TPP toward the live baseline, then add the independent
	// election, region and seat deviations supplied by LiveV2. Deviations are
	// additive on the transformed vote-share scale.
	auto const electionTppInformation =
		liveElection->getFinalSpecificTppInformation();
	if (!std::isfinite(electionTppInformation.deviation)) {
		throw InvalidIteration();
	}
	for (int seatIndex = 0;
		seatIndex < project.seats().count(); ++seatIndex) {
		auto const& seat = project.seats().viewByIndex(seatIndex);
		auto const seatInformation =
			liveElection->getSeatTppInformation(seat.name);
		int const regionIndex =
			project.regions().idToIndex(seat.region);
		if (regionIndex == RegionCollection::InvalidIndex) {
			throw InvalidIteration();
		}
		float const regionDeviation =
			liveElection->getRegionFinalSpecificTppDeviation(regionIndex);
		if (!std::isfinite(seatInformation.baseline) ||
			!std::isfinite(seatInformation.deviation) ||
			!std::isfinite(regionDeviation)) {
			throw InvalidIteration();
		}

		float const baselineWeight = liveBlendWeight(
			seatInformation.completion, seatInformation.confidence);
		float const totalDeviation =
			electionTppInformation.deviation +
			regionDeviation +
			seatInformation.deviation;
		if (baselineWeight > 0.0f || materiallyNonZero(totalDeviation)) {
			float const baselineMargin =
				detransformVoteShare(seatInformation.baseline) - 50.0f;
			float const mixedMargin = mix(
				partyOneNewTppMargin[seatIndex],
				baselineMargin,
				baselineWeight);
			float transformedTpp =
				transformVoteShare(mixedMargin + 50.0f);
			transformedTpp += totalDeviation;
			partyOneNewTppMargin[seatIndex] =
	...
	auto const electionFpDeviations =
		liveElection->getFinalSpecificFpDeviations();
	for (int seatIndex = 0; seatIndex < project.seats().count(); ++seatIndex) {
		auto const& seat = project.seats().viewByIndex(seatIndex);
		int const regionIndex =
			project.regions().idToIndex(seat.region);
		...
		auto const regionFpDeviations =
			liveElection->getRegionFinalSpecificFpDeviations(regionIndex);
		auto const seatFpInformation =
			liveElection->getSeatFpInformation(seat.name);
		bool requiresNormalisation = false;

		auto partyHasPositiveShare = [&](int partyIndex) {
			return seatFpVoteShare[seatIndex].contains(partyIndex) &&
				seatFpVoteShare[seatIndex].at(partyIndex) >
				LiveSimulationMath::EvidenceEpsilon;
		};

		auto effectivePartyIndex = [&](int partyIndex) {
			if (partyIndex == run.indPartyIndex &&
				!partyHasPositiveShare(run.indPartyIndex) &&
				partyHasPositiveShare(EmergingIndIndex)) {
				return EmergingIndIndex;
			}
			return partyIndex;
		};
		auto deviationForParty = [](auto const& deviations, int partyIndex) {
			auto const it = deviations.find(partyIndex);
			return it == deviations.end() ? 0.0f : it->second;
		};

		// The standard path keeps Liberal and National votes combined until TCP
		// resolution. Split them early only when National-specific live evidence
		// actually needs a separate share.
		bool coalitionWasSplit = false;
		if (run.natPartyIndex >= 0) {
			float nationalsBaselineWeight = 0.0f;
			auto const nationalsInfo =
				seatFpInformation.find(run.natPartyIndex);
			if (nationalsInfo != seatFpInformation.end()) {
				nationalsBaselineWeight = liveBlendWeight(
					nationalsInfo->second.completion,
					nationalsInfo->second.confidence);
			}
			auto const nationalsProportion =
				liveElection->getSeatNationalsProportion(seat.name);
			float nationalsProportionWeight = 0.0f;
			if (nationalsProportion) {
				nationalsProportionWeight = liveBlendWeight(
					nationalsProportion->completion,
					nationalsProportion->confidence,
					14.0f);
			}
			bool const hasNationalsDeviation =
				materiallyNonZero(deviationForParty(
					electionFpDeviations, run.natPartyIndex)) ||
				materiallyNonZero(deviationForParty(
					regionFpDeviations, run.natPartyIndex)) ||
				(nationalsInfo != seatFpInformation.end() &&
					materiallyNonZero(nationalsInfo->second.deviation));
			if (nationalsBaselineWeight > 0.0f ||
				nationalsProportionWeight > 0.0f ||
				hasNationalsDeviation) {
				assignNationalsVotes(
					seatIndex, nationalsProportionWeight > 0.0f);
				coalitionWasSplit = true;
			}
		}

		auto applyFpDeviation = [&](int partyIndex, float deviation) {
			if (!std::isfinite(deviation)) throw InvalidIteration();
			if (!materiallyNonZero(deviation)) return;
			int const effectiveIndex =
				effectivePartyIndex(partyIndex);
			if (!partyHasPositiveShare(effectiveIndex)) return;
			float transformedFp = transformVoteShare(
				seatFpVoteShare[seatIndex].at(effectiveIndex));
			transformedFp += deviation;
			seatFpVoteShare[seatIndex][effectiveIndex] =
				detransformVoteShare(transformedFp);
			requiresNormalisation = true;
		};

		// As confidence rises, replace prior seat-level variation with the
		// randomized live baseline before applying observed deviations.
		struct PendingPartyActivation {
			int partyIndex;
			float targetShare;
			float weight;
		};
		std::vector<PendingPartyActivation> pendingActivations;
		for (auto const& [partyIndex, information] : seatFpInformation) {
			if (!std::isfinite(information.baseline)) {
				throw InvalidIteration();
			}
			float const baselineWeight = liveBlendWeight(
				information.completion, information.confidence);
			if (baselineWeight == 0.0f) continue;

			int const effectiveIndex =
				effectivePartyIndex(partyIndex);
			if (partyHasPositiveShare(effectiveIndex)) {
				float const priorFpShare = transformVoteShare(std::clamp(
					seatFpVoteShare[seatIndex].at(effectiveIndex),
					0.01f, 99.99f));
				float const mixedFpShare = mix(
					priorFpShare, information.baseline, baselineWeight);
				seatFpVoteShare[seatIndex][effectiveIndex] =
					detransformVoteShare(mixedFpShare);
				requiresNormalisation = true;
				continue;
			}

			bool const configuredParty =
				effectiveIndex >= 0 &&
				effectiveIndex < project.parties().count();
			bool const independentProxy =
				effectiveIndex == EmergingIndIndex;
			if (!configuredParty && !independentProxy) continue;
			pendingActivations.push_back({
				effectiveIndex,
				detransformVoteShare(information.baseline),
				baselineWeight,
			});
		}

		// Retain a strength-weighted portion of the generic OTH pool for
		// candidates that remain unrepresented. A strongly observed new party
		// receives more than one generic candidate's implied share, while direct
		// live OTH evidence below replaces this prior progressively.
		int remainingOthersCandidates = run.othCount[seatIndex];
		if (partyHasPositiveShare(EmergingPartyIndex)) {
			--remainingOthersCandidates;
		}
		int remainingIndependentCandidates = run.indCount[seatIndex];
		if (partyHasPositiveShare(run.indPartyIndex)) {
			--remainingIndependentCandidates;
		}
		if (partyHasPositiveShare(EmergingIndIndex)) {
			--remainingIndependentCandidates;
		}
		std::vector<float> othersActivationShares;
		for (auto const& activation : pendingActivations) {
			if (activation.partyIndex == run.natPartyIndex) continue;
			othersActivationShares.push_back(
				activation.targetShare * activation.weight);
			if (activation.partyIndex == EmergingPartyIndex) {
				--remainingOthersCandidates;
			}
			if (activation.partyIndex == run.indPartyIndex ||
				activation.partyIndex == EmergingIndIndex) {
				--remainingIndependentCandidates;
			}
		}
		int const remainingGenericCandidates =
			std::max(0, remainingOthersCandidates) +
			std::max(0, remainingIndependentCandidates);
		float const priorOthersShare = getAt(
			seatFpVoteShare[seatIndex], OthersIndex, 0.0f);
		float const othersReserve = LiveSimulationMath::othersReserve(
			priorOthersShare,
			remainingGenericCandidates,
			othersActivationShares);

		for (auto const& pending : pendingActivations) {
			int const sourceIndex =
				pending.partyIndex == run.natPartyIndex ?
				Mp::Two : OthersIndex;
			float const sourceShare = getAt(
				seatFpVoteShare[seatIndex], sourceIndex, 0.0f);
			auto const activation = LiveSimulationMath::activateFromSource(
				pending.targetShare,
				pending.weight,
				sourceShare,
				sourceIndex == OthersIndex ? othersReserve : 0.0f);
			if (activation.partyShare <= 0.0f) continue;
			seatFpVoteShare[seatIndex][pending.partyIndex] =
				activation.partyShare;
			seatFpVoteShare[seatIndex][sourceIndex] =
				activation.sourceShare;
			requiresNormalisation =
				requiresNormalisation || activation.requiresNormalisation;
		}
		for (auto const& [partyIndex, deviation] :
			electionFpDeviations) {
			applyFpDeviation(partyIndex, deviation);
		}
		for (auto const& [partyIndex, deviation] :
			regionFpDeviations) {
			applyFpDeviation(partyIndex, deviation);
		}
		for (auto const& [partyIndex, information] :
			seatFpInformation) {
			applyFpDeviation(partyIndex, information.deviation);
		}

		float seatEvidenceWeight = 0.0f;
		for (auto const& [_, information] : seatFpInformation) {
			seatEvidenceWeight = std::max(
				seatEvidenceWeight,
				liveBlendWeight(
					information.completion, information.confidence));
		}

		// LiveV2 tracks one principal independent. Preserve the second standard
		// independent when the ballot contains at least two; otherwise phase the
		// proxy into Others as live evidence accumulates.
		if (run.indCount[seatIndex] < 2 &&
			partyHasPositiveShare(EmergingIndIndex) &&
			partyHasPositiveShare(run.indPartyIndex) &&
			seatEvidenceWeight > 0.0f) {
			float const previousShare =
				seatFpVoteShare[seatIndex][EmergingIndIndex];
			float const removedShare =
				previousShare * seatEvidenceWeight;
			seatFpVoteShare[seatIndex][EmergingIndIndex] =
				previousShare - removedShare;
			seatFpVoteShare[seatIndex][OthersIndex] += removedShare;
		}

		// Recalculate major-party FPs here if live preference-flow information
		// is integrated in future.
		...
		// Recombine the Coalition for preference allocation. It will be split
		// again when the final pair of candidates is determined. Retain the
		// post-deviation ratio so that split does not discard the separate
		// Liberal and National live FP adjustments just applied above.
		if (coalitionWasSplit) {
			float const coalitionVote =
				seatFpVoteShare[seatIndex][Mp::Two] +
				seatFpVoteShare[seatIndex][run.natPartyIndex];
			if (coalitionVote > 0.0f) {
				nationalsShare[seatIndex] = std::clamp(
					seatFpVoteShare[seatIndex][run.natPartyIndex] /
						coalitionVote,
					0.0f, 1.0f);
			}
			seatFpVoteShare[seatIndex][Mp::Two] = coalitionVote;
			seatFpVoteShare[seatIndex].erase(run.natPartyIndex);
		}
		if (partyHasPositiveShare(EmergingPartyIndex) &&
			seatEvidenceWeight > 0.0f) {
			float const previousShare =
				seatFpVoteShare[seatIndex][EmergingPartyIndex];
			float const removedShare =
				previousShare * seatEvidenceWeight;
			seatFpVoteShare[seatIndex][EmergingPartyIndex] =
				previousShare - removedShare;
		...
		std::map<int, float> representedParties;
		for (auto const& [partyIndex, share] :
			seatFpVoteShare[seatIndex]) {
			if (share > LiveSimulationMath::EvidenceEpsilon) {
				representedParties[partyIndex] = share;
			}
		}
		if (!representedParties.contains(run.indPartyIndex) &&
			representedParties.contains(EmergingIndIndex)) {
		...
		if (!std::isfinite(seatOthersInformation.value) ||
			seatOthersInformation.value < 0.0f ||
			seatOthersInformation.value > 100.0f) {
			throw InvalidIteration();
		}
		float const observedWeight = liveBlendWeight(
			seatOthersInformation.completion,
			seatOthersInformation.confidence);
		bool const hasOthersInformation =
			seatOthersInformation.value > 0.0f ||
			seatOthersInformation.completion > 0.0f ||
			seatOthersInformation.confidence > 0.0f;
		if (hasOthersInformation && observedWeight > 0.0f) {
			seatFpVoteShare[seatIndex][OthersIndex] = mix(
				getAt(
					seatFpVoteShare[seatIndex],
					OthersIndex, 0.0f),
				seatOthersInformation.value,
				observedWeight);
			requiresNormalisation = true;
		}
		if (requiresNormalisation) {
			std::set<int> fixedLiveParties;
			if (othersReserve > 0.0f) {
				fixedLiveParties.insert(OthersIndex);
				for (auto const& activation : pendingActivations) {
					if (activation.partyIndex != run.natPartyIndex &&
						seatFpVoteShare[seatIndex].contains(
							activation.partyIndex)) {
						fixedLiveParties.insert(activation.partyIndex);
					}
				}
			}

			if (fixedLiveParties.empty()) {
				normaliseSeatFp(seatIndex);
			}
			else {
				float fixedVote = 0.0f;
				float flexibleVote = 0.0f;
				for (auto const& [partyIndex, voteShare] :
					seatFpVoteShare[seatIndex]) {
					if (partyIndex == CoalitionPartnerIndex) continue;
					if (fixedLiveParties.contains(partyIndex)) {
						fixedVote += voteShare;
					}
					else {
						flexibleVote += voteShare;
					}
				}
				float const flexibleTarget = 100.0f - fixedVote;
				if (!std::isfinite(fixedVote) ||
					!std::isfinite(flexibleVote) ||
					flexibleVote <= 0.0f || flexibleTarget < 0.0f) {
					throw InvalidIteration();
				}
				float const correctionFactor =
					flexibleTarget / flexibleVote;
				for (auto& [partyIndex, voteShare] :
					seatFpVoteShare[seatIndex]) {
					if (partyIndex == CoalitionPartnerIndex ||
						fixedLiveParties.contains(partyIndex)) {
						continue;
					}
					voteShare *= correctionFactor;
				}
...
{
	Seat const& seat = project.seats().viewByIndex(seatIndex);
	using PartyVotes = std::pair<int, float>;
	auto partyVoteLess = [](PartyVotes const& a, PartyVotes const& b) {
		return a.second < b.second;
	};

	// Transfer FP shares into the candidate-elimination count. Generic Others
	// is not a candidate, so its preferences are distributed from the outset.
	std::vector<PartyVotes> originalVoteShares;
	...
	for (auto const& partyVote : seatFpVoteShare[seatIndex]) {
		if (partyVote.second == 0.0f) continue;
		if (partyVote.first == OthersIndex) {
			// always exclude "Others" before considering other candidates
			excludedVoteShares.push_back(partyVote);
		}
		else {
			originalVoteShares.push_back(partyVote);
		}
	}
	if (originalVoteShares.size() < 2) {
		throw InvalidIteration();
	}
	...
	// Allocate all already-excluded votes among candidates remaining in the
	// count. The function is rerun after each exclusion because destinations
	// and known non-classic preference flows depend on the surviving pair.
	auto allocateVotes = [&](std::vector<PartyVotes>& accumulatedVoteShares, std::vector<PartyVotes> const& excludedVoteShares) {
		if (accumulatedVoteShares.empty()) {
			throw InvalidIteration();
		}
		for (auto [sourceParty, sourceVoteShare] : excludedVoteShares) {
			...
			float const sourceExhaustRate =
				getAt(overallExhaustRate, sourceParty, 0.0f);
			float survivalRate = sourceExhaustRate ?
				(1.0f - sourceExhaustRate) * 0.85f : 1.0f;
			...
			if (sourceParty == Mp::One &&
				getAt(overallExhaustRate, OthersIndex, 0.0f) > 0.01f) {
				survivalRate = 0.5f;
			}
			else if (sourceParty == Mp::Two &&
				getAt(overallExhaustRate, OthersIndex, 0.0f) > 0.01f) {
				survivalRate = 0.4f;
			}
					...
					if (item.contains(targetParties)) {
						...
						if (targetParties.first >= 0 && targetParties.second >= 0) {
							...
							if (run.seatTypes.at(seatIndex) == SimulationRun::SeatType::Provincial || run.seatTypes.at(seatIndex) == SimulationRun::SeatType::Rural) {
								if (sourceParty == Mp::Two && project.parties().viewByIndex(targetParties.first).abbreviation == "ON" && targetParties.second == Mp::One) flow += 5.0f + regionalBoost;
								if (sourceParty == Mp::Two && project.parties().viewByIndex(targetParties.second).abbreviation == "ON" && targetParties.first == Mp::One) flow -= 5.0f + regionalBoost;
							}
							else {
								if (sourceParty == Mp::Two && project.parties().viewByIndex(targetParties.first).abbreviation == "ON" && targetParties.second == Mp::One) flow -= 5.0f + regionalBoost;
								if (sourceParty == Mp::Two && project.parties().viewByIndex(targetParties.second).abbreviation == "ON" && targetParties.first == Mp::One) flow += 5.0f + regionalBoost;
						...
						if (seat.name == "Kiama" && run.getTermCode() == "2023nsw" && sourceParty == Mp::One) {
							flow = 50.0f;
							survivalRate = 0.2f;
						}
						if ((seat.name == "Narungga" || seat.name == "MacKillop") && run.getTermCode() == "2026sa" && sourceParty == Mp::One) {
							flow = 50.0f;
						}
			...
			for (int targetIndex = 0; targetIndex < int(accumulatedVoteShares.size()); ++targetIndex) {
				...
				if (targetParty == Mp::One) alpIndex = targetIndex;
				if (targetParty == Mp::Two ||
					targetParty == CoalitionPartnerIndex) {
					lnpIndex = targetIndex;
				}
				if (targetParty == run.grnPartyIndex) grnIndex = targetIndex;
				if (!partyIdeologies.contains(sourceParty) ||
					!partyIdeologies.contains(targetParty) ||
					!partyConsistencies.contains(sourceParty)) {
					throw InvalidIteration();
				}
				if (targetParty == run.indPartyIndex ||
					partyIdeologies.at(targetParty) == 2) {
					indIndex = targetIndex;
				}
				int ideologyDistance = std::abs(
					partyIdeologies.at(sourceParty) -
					partyIdeologies.at(targetParty));
				if (isMajor(targetParty)) ++ideologyDistance;
				int const consistency = partyConsistencies.at(sourceParty);
				if (consistency < 0 ||
					consistency >= int(PreferenceConsistencyBase.size())) {
					throw InvalidIteration();
				}
				float consistencyBase = PreferenceConsistencyBase[consistency];
			...
			if (alpIndex >= 0 && grnIndex >= 0 &&
				sourceParty == OthersIndex) {
			...
			if ((seat.name == "Narungga" || seat.name == "MacKillop") &&
				run.getTermCode() == "2026sa" &&
				sourceParty == Mp::One && indIndex >= 0) {
				// Prevent ALP votes from heavily favouring these specific INDs and pushing them into TCP
				weights[indIndex] *= 0.2f;
			}

			constexpr float WeightEpsilon = 0.0000001f;
			float totalWeight =
				std::accumulate(
					weights.begin(), weights.end(), WeightEpsilon);
			if (!std::isfinite(totalWeight) ||
				totalWeight <= WeightEpsilon) {
				throw InvalidIteration();
			}
			if ((sourceParty == CoalitionPartnerIndex ||
				sourceParty == Mp::Two) && lnpIndex != -1) {
	...
	// Exclude the lowest candidate each round until the final pair is fixed.
	// Each non-final pass removes one candidate, so the loop is bounded by the
	...
	while (true) {
		...
		if (finalTwoConfirmed) {
			...
			if (accumulatedVoteShares[0].first == Mp::One && (accumulatedVoteShares[1].first == Mp::Two || accumulatedVoteShares[1].first == CoalitionPartnerIndex)) {
				...
				// The leading pair can be mathematically confirmed before every
				// trailing candidate has been formally excluded.
				accumulatedVoteShares.resize(2);
				break;
			}
			...
			float const finalVoteTotal =
				accumulatedVoteShares[0].second +
				accumulatedVoteShares[1].second;
			if (!std::isfinite(finalVoteTotal) || finalVoteTotal <= 0.0f) {
				throw InvalidIteration();
			}
			float adjustmentFactor = 100.0f / finalVoteTotal;
	...
	std::pair<PartyVotes, PartyVotes> topTwo =
		std::minmax(
			accumulatedVoteShares[0],
			accumulatedVoteShares[1],
			partyVoteLess);
	if (topTwo.first.first == CoalitionPartnerIndex) {
		topTwo.first.first = Mp::Two;
	}
	if (topTwo.second.first == CoalitionPartnerIndex) {
	...
	if (run.natPartyIndex >= 0 && nationalsShare[seatIndex] > 0) {
		...
		if (!std::isfinite(coalitionShare) || coalitionShare <= 0.0f) {
			throw InvalidIteration();
		}
		float lowerShare = std::min(natShare, libShare) / coalitionShare;
		if (
	...
	if (run.isLiveAutomatic() && !(isMajor(topTwo.first.first, run.natPartyIndex) && isMajor(topTwo.second.first, run.natPartyIndex))) {
		...
		if (tcpInfo.shares.contains(topTwo.first.first) && tcpInfo.shares.contains(topTwo.second.first)) {
			...
			float effectiveConfidence = effectiveLiveConfidence(
				tcpInfo.completion, tcpInfo.confidence);
			...
			float baselineWeight = LiveSimulationMath::evidenceWeight(
				effectiveConfidence, 14.0f);
			if (baselineWeight > 0.0f) {
				float const priorShare =
					transformVoteShare(topTwo.first.second);
				float const mixedShare =
					mix(priorShare, liveShare, baselineWeight);
	...
	auto validContestParty = [&](int partyIndex) {
		return partyIndex == EmergingIndIndex ||
			partyIndex == EmergingPartyIndex ||
			(partyIndex >= 0 &&
				partyIndex < project.parties().count());
	};
	if (!validContestParty(topTwo.first.first) ||
		!validContestParty(topTwo.second.first) ||
		topTwo.first.first == topTwo.second.first ||
		!std::isfinite(topTwo.first.second) ||
		!std::isfinite(topTwo.second.second) ||
		topTwo.first.second < 0.0f ||
		topTwo.first.second > 100.0f ||
		topTwo.second.second < 0.0f ||
		topTwo.second.second > 100.0f) {
		throw InvalidIteration();
	}
...
void SimulationIteration::assignNationalsVotes(
	...
	auto const& seat = project.seats().viewByIndex(seatIndex);
	if (updateFromLive && run.isLiveAutomatic()) {
		...
		if (nationalsProportion && nationalsShare[seatIndex] > 0.0f && nationalsShare[seatIndex] < 1.0f) {
			...
			if (liveEstimate > 0.0f && liveEstimate < 1.0f) {
				const float effectiveConfidence = effectiveLiveConfidence(
					nationalsProportion.value().completion,
					nationalsProportion.value().confidence);
				...
				const float liveWeight = LiveSimulationMath::evidenceWeight(
					effectiveConfidence, 14.0f);
...
{
	// Create entries for every reportable party even when it wins no seats.
	for (int partyIndex = 0; partyIndex < project.parties().count(); ++partyIndex) {
		partyWins[partyIndex] = 0;
		regionSeatCount[partyIndex] =
			std::vector<int>(project.regions().count());
	}
	partyWins[EmergingPartyIndex] = 0;
	regionSeatCount[EmergingPartyIndex] =
		std::vector<int>(project.regions().count());
	...
	for (int seatIndex = 0; seatIndex < project.seats().count(); ++seatIndex) {
		...
		if (!partyWins.contains(partyIndex)) {
			throw InvalidIteration();
		}
		int const regionIndex =
			project.regions().idToIndex(seat.region);
...
void SimulationIteration::assignSupportsPartyWins()
{
	...
	for (int partyNum = Mp::Others; partyNum < project.parties().count(); ++partyNum) {
		...
		int const target = thisParty.relationTarget;
		// Parliamentary relationships are currently only modelled against one
		// of the two major parties.
		if (target < Mp::One || target > Mp::Two) continue;
		switch (thisParty.relationType) {
		case Party::RelationType::IsPartOf:
		case Party::RelationType::Coalition:
			effectiveWins[target] += partyWins.at(partyNum);
			partySupport[target] += partyWins.at(partyNum);
			break;
...
void SimulationIteration::recordPartySeatWinCounts()
{
	...
	for (auto [partyIndex, wins] : partyWins) {
		...
		++sim.latestReport.partySeatWinFrequency[partyIndex][wins];
		if (!isMajor(partyIndex, run.natPartyIndex)) {
			othersWins += wins;
		}
...
void SimulationIteration::recordRegionFpVotes(int regionIndex)
{
	...
	for (int seatIndex = 0; seatIndex < project.seats().count(); ++seatIndex) {
		auto const& seat = project.seats().viewByIndex(seatIndex);
		if (project.regions().idToIndex(seat.region) != regionIndex) {
			continue;
		}
		double const turnout =
			double(run.pastSeatResults[seatIndex].turnoutCount);
		for (auto [partyIndex, fpPercent] : seatFpVoteShare[seatIndex]) {
			weightedVoteShare[partyIndex] += fpPercent * turnout;
		}
		turnoutSum += turnout;
	}
...
void SimulationIteration::recordRegionTppVotes(int regionIndex)
{
	...
	for (int seatIndex = 0; seatIndex < project.seats().count(); ++seatIndex) {
		...
		double const turnout =
			double(run.pastSeatResults[seatIndex].turnoutCount);
		weightedTpp +=
			double(partyOneNewTppMargin[seatIndex] + 50.0f) *
			turnout;
		turnoutSum += turnout;
	}
	if (turnoutSum <= 0.0) return;
...
{
	// Reconciliation targets and published election totals are turnout-weighted;
	// the live-baseline aggregate must use the same definition.
	std::map<int, double> weightedVoteShare;
	double turnoutSum = 0.0;
	for (int seatIndex = 0; seatIndex < project.seats().count(); ++seatIndex) {
		double const turnout =
