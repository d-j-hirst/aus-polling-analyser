...
void DisplayFrameRenderer::drawRegionsBoxRowListItem(RegionCollection::Index regionIndex, wxRect rowNameRect) const
{
	...
	float change[3] = { seats[0] - simulation.regionPartyLeading[regionIndex][0] ,
		seats[1] - simulation.regionPartyLeading[regionIndex][1],
...
void DisplayFrameRenderer::drawVoteShareBoxRow(wxRect& nameRect, PartyCollection::Index partyIndex) const
{
	...
	if (partyIndex < int(simulation.internalPartyCount())) {
		...
		if (partyIndex < int(simulation.partyPrimaryFrequency.size()) && simulation.getPrimarySampleCount(partyIndex)) {
			...
			float median = simulation.getPrimarySampleMedian(partyIndex);
